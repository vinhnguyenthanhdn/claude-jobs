...
- **macOS, Linux, and any Unix with cron.** The scheduler is launchd on macOS, systemd user timers on Linux, and cron where either is missing. Windows Task Scheduler is not supported. Of those three, CI hands a real unit to **systemd** and reads it back from the scheduler — registered, enabled, active, carrying the `--at` value, and gone again after `uninstall`. launchd and cron are covered only at the level of the files and crontab lines this tool writes; no CI leg has asked `launchctl` or `crond` whether it accepted them.
