...
- **macOS, Linux, and any Unix with cron.** The scheduler is launchd on macOS, systemd user timers on Linux, and cron where either is missing. Windows Task Scheduler is not supported. CI exercises **launchd**, **systemd**, and **cron** against the schedulers that own them rather than against the files this tool writes: a launchd job is registered in the user's GUI domain and read back with `launchctl`, a systemd timer is registered, enabled, active and carrying the `--at` value, and a crontab line is read back from `crontab -l` — all gone again after `uninstall`.
