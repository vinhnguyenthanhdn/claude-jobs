...
- **macOS, Linux, and any Unix with cron.** The scheduler is launchd on macOS, systemd user timers on Linux, and cron where either is missing. Windows Task Scheduler is not supported. CI exercises **systemd** and **cron** against the scheduler that owns them rather than against the files this tool writes: a real timer registered, enabled, active and carrying the `--at` value, and a real crontab line read back from `crontab -l` — both gone again after `uninstall`. launchd is still covered only at the level of the plist this tool writes; no CI leg has asked `launchctl` whether it accepted one.
