...
function findClaudeBinary() {
  const found = spawnSync("/bin/sh", ["-c", "command -v claude"], {
    encoding: "utf8",
  });
...
export function cmdInit(args, flags) {
  ...
  if (existsSync(jobDir(name)) && !flags.force) {
    throw new Error(
      `job "${name}" already exists. Pass --force to overwrite it.`,
    );
  }
  if (flags.jitter !== undefined) {
    if (
      typeof flags.jitter === "boolean" ||
      !/^\d+$/.test(String(flags.jitter))
    ) {
      throw new Error(
        `--jitter must be a non-negative integer. Received "${flags.jitter}".`,
      );
    }
  }
  ...
  if (flags["prompt-file"] !== undefined) {
    if (
      typeof flags["prompt-file"] === "boolean" ||
      !existsSync(resolve(flags["prompt-file"]))
    ) {
      throw new Error(`--prompt-file not found: "${flags["prompt-file"]}".`);
    }
  }
  if (flags.scheduler !== undefined) {
    if (!["launchd", "systemd", "cron"].includes(flags.scheduler)) {
      throw new Error(
        `unknown scheduler "${flags.scheduler}". Use launchd, systemd or cron.`,
      );
    }
  }
  ensureDirs();
  ...
  writeFileSync(
    promptFile(name),
    render(readPromptTemplate(flags), {
      TASK: job.task,
...
function readPromptTemplate(flags) {
  if (flags["prompt-file"])
    return readFileSync(resolve(flags["prompt-file"]), "utf8");
...
export function cmdList() {
  ...
  if (names.length === 0) {
    console.log(
      "No jobs yet. Create one with: claude-jobs init <name> --skill <path>",
    );
    return;
  }
  console.log(
    "NAME".padEnd(24) + "AT".padEnd(8) + "SCHEDULER".padEnd(12) + "INSTALLED",
  );
...
export function cmdRun(args, flags) {
  ...
  execFileSync("/bin/bash", [runnerFile(job.name), ...runnerArgs], {
    stdio: "inherit",
  });
}
...
export function cmdStatus(args) {
  ...
  console.log(
    `schedule   ${String(job.hour).padStart(2, "0")}:${String(job.minute).padStart(2, "0")} daily (+ up to ${job.jitter}s jitter)`,
  );
  console.log(
    `scheduler  ${job.scheduler} (${isInstalled(job) ? "installed" : "not installed"})`,
  );
...
export function cmdDoctor() {
  ...
  checks.push([
    existsSync(claude) || claude !== "claude",
  ...
  if (!checks.every(([ok]) => ok)) {
    console.log("");
    console.log(
      'Fix: install Claude Code, then run "claude auth login" as the same user that runs the jobs.',
    );
  }
}
